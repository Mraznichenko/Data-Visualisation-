...
df = df.where(df['trip_duration']<=2000)
df = df.dropna()
...
df['dates'] = pd.to_datetime(df['pickup_datetime']).dt.date
#Color operations
...
df_scatterplot = pd.concat([df['trip_duration'], df['dates'], df['vendor_id'], df['color'], df['passenger_count']], axis=1,keys=["trip_duration", "dates", "vendor_id", "color", "passenger_count"])

df_scatterplot = df_scatterplot.sort_values(by=['dates'])
...
# data = df_scatterplot.to_dict('series')
data = {'dates':df_scatterplot['dates'].values, 'trip_duration':df_scatterplot['trip_duration'].values, 'color':df_scatterplot['color'].values, 'passenger_count':df_scatterplot['passenger_count'].values, 'vendor_id':df_scatterplot['vendor_id'].values}
...
p = figure(tools=TOOLS, plot_width=3000, plot_height=900,
           toolbar_location="above",x_range = x_Range2,
...
hover = HoverTool(tooltips = [
    ("Date:", "@dates"),("Duration:", "@trip_duration"),("Number of passengers:", "@passenger_count"), ("Vendor_id:", "@vendor_id")
])
p.add_tools(hover)
#Create the scatterplot, be aware that the size of the circles should encode the number of passengers.
scatter = p.scatter(x='dates', y='trip_duration', size='passenger_count', color = 'color', source = source_scatter)
...
hhist, hedges = np.histogram(trip_duration, density=False, bins=20)
hzeros = np.zeros(len(hedges)-1)
hmax = max(hhist)*1.1
LINE_ARGS1 = dict(color="#32a852", line_color=None)
LINE_ARGS2 = dict(color="#5c0c19", line_color=None)
...
ph.quad(top=hhist, bottom = 0, left=hedges[:-1], right=hedges[1:])
ph.y_range.start = 0
ph.yaxis.axis_label = "Number of Trips"
ph.xaxis.axis_label = "Trip Duration"
hh1 = ph.quad(top = 0, bottom=0, left=hedges[:-1], right=hedges[1:], alpha=0.5, **LINE_ARGS1)
hh2 = ph.quad(top = 0, bottom=0, left=hedges[:-1], right=hedges[1:], alpha=0.5, **LINE_ARGS2)
...
def update(attr, old, new):
    ...
    j = max(nds)
    while i<j:
        if(data['vendor_id'][i]=='vendor_1'):
            hh1arr.append(data['trip_duration'][i])
        else:
            hh2arr.append(data['trip_duration'][i])
        i+=1
    ...
    ph2 = figure(title="Histogram", tools='', background_fill_color="#fafafa", plot_width=1500, plot_height=200,
                x_range=p.y_range, y_range=(0, hmax))
    ph2.xgrid.grid_line_color = None
    ph2.yaxis.major_label_orientation = np.pi / 4
    ph2.quad(top=hhist, bottom=0, left=hedges[:-1], right=hedges[1:])
    ph2.y_range.start = 0
    ph2.yaxis.axis_label = "Number of Trips"
    ph2.xaxis.axis_label = "Trip Duration"
    hhist2, hedges2 = np.histogram(hh2arr, density=False, bins=20)
    hhist1, hedges1 = np.histogram(hh1arr, density=False, bins=20)
    ph2.quad(top=hhist1, bottom=0, left=hedges1[:-1], right=hedges1[1:], alpha=0.5, **LINE_ARGS1)
    ph2.quad(top=hhist2, bottom=0, left=hedges2[:-1], right=hedges2[1:], alpha=0.5, **LINE_ARGS2)
    curdoc().clear()
    layout = column(p, row(ph2))
...
# bokeh serve --show dvc_ex3_maznichenko_lev.py